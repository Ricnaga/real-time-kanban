  ...
  updates: {
    Subscription: {
      ...
      taskMoved: (result, _args, cache) => {
        ...
        const movedTaskIds = new Set(tasks.map((t) => t.id));
        ...
        for (const action of actionsData.actions) {
          ...
          if (updatedTasks) {
            ...
            cache.updateQuery<TasksByActionData>(
              ...
              (data) => {
                if (!data) return data;
                const remaining = data.tasksByAction.filter(
                  (t) => !movedTaskIds.has(t.id),
                );
                if (remaining.length === data.tasksByAction.length) {
                  return data;
                }
                return { tasksByAction: remaining };
              },
            );
          }
        }
